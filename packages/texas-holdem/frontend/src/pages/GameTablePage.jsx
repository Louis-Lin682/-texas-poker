    ...
    <button type="button" onClick={onClick} disabled={disabled} style={{
      ...
      {amount != null && (
        <span style={{
          ...
          textShadow: '0 1px 3px #000', pointerEvents: 'none', ...amountStyle,
        }}>
          {amount}
        </span>
      )}
...
function evalHand(cards) {
  ...
  const sFreq  = {}; suits.forEach(s => { sFreq[s]=(sFreq[s]||0)+1 })
...
function LobbyView({ status, rooms, onCreateRoom, onJoinRoom, onRefresh, onLogout }) {
  ...
  return (
    <div className="pt-lobby">
      ...
      <button type="button" className="pt-logout-btn" onClick={onLogout}>登出</button>
    </div>
  )
}
...
// ── Main ──────────────────────────────────────────────────────────────────────
function GameTablePage({ auth, onLogout }) {
  const minBuyIn     = 3000
  ...
  useEffect(() => {
    if (!wasKicked) return
    const t = setTimeout(() => onLogout(), 3000)
  ...
  useEffect(() => {
    ...
    if (cur === 'pre_flop' && prev !== 'pre_flop') setDealKey(k => k + 1)
  ...
  useEffect(() => {
    ...
    if (bal !== null && bal === 0 && prevBalRef.current > 0 && phase === 'waiting') {
      setShowRebuyModal(true)
    }
    if (bal !== null) prevBalRef.current = bal
  ...
  useEffect(() => {
    ...
    const onPop = () => {
      window.history.pushState(null, '', window.location.href)
      if (roomId) setShowLeaveConfirm(true)
      else onLogout()
    }
  ...
  return (
    <div className="pt-page">
      ...
      {showLeaveConfirm && (
        <LeaveConfirmModal
          body="遊戲進行中，離開後本局視為棄牌，已下注籌碼將無法取回。"
        ...
        <div className="pt-modal-overlay">
          <div className="pt-modal">
            ...
            <div className="pt-modal-btns">
              ...
              <button type="button" className="pt-modal-confirm" onClick={() => handleRebuy(minBuyIn)}>
                補充 {new Intl.NumberFormat('en-US').format(minBuyIn)} 籌碼
              </button>
            </div>
          </div>
        </div>
      ...
      <header className="pt-header">
        ...
        <button type="button" className="pt-mute-btn" onClick={() => setIsGameMuted(m => !m)}>
