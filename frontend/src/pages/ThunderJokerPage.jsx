...
export default function ThunderJokerPage({ auth }) {
  ...
  const initServerBalRef = useRef(buyIn != null && auth?.user?.balance != null ? auth.user.balance : null)
  ...
  const buyInLoggedRef  = useRef(false)
  ...
  useEffect(() => {
    if (buyIn != null) return          // started with a specific buy-in; server spin responses take over after first spin
    if (auth?.user?.balance != null) {
      balRef.current = auth.user.balance
      setBalance(auth.user.balance)
      setDisplayBalance(auth.user.balance)
    }
  }, [auth?.user?.balance, buyIn])

  // In buy-in mode, ensure initServerBalRef is set to the real server balance as soon as auth is ready.
  // The useRef initializer only runs once and may have seen null if auth hadn't loaded yet.
  useEffect(() => {
    if (buyIn == null || initServerBalRef.current != null) return
    if (auth?.user?.balance != null) {
      initServerBalRef.current = auth.user.balance
    }
  }, [auth?.user?.balance, buyIn])
  ...
  // Record buy_in ledger entry once on session start (buy-in mode only)
  useEffect(() => {
    if (buyIn == null || buyInLoggedRef.current) return
    const token = auth?.token
    if (!token) return
    buyInLoggedRef.current = true
    apiRequest('/slots/ledger', {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` },
      body: JSON.stringify({ type: 'buy_in', amount: -buyIn }),
    }).catch(() => {})
  }, [auth?.token, buyIn])
  ...
  const doSpin = useCallback(() => {
    ...
    function finalize() {
      ...
      // If auth wasn't ready at mount, reconstruct the initial server balance from this spin result
      if (buyIn != null && initServerBalRef.current == null && !isFree) {
        initServerBalRef.current = newBalance + curBet - winTotal
      }
      // Authoritative balance: in buy-in mode show session delta, otherwise show real balance
      const displayBal = (buyIn != null && initServerBalRef.current != null)
        ? Math.max(0, buyIn + (newBalance - initServerBalRef.current))
        : newBalance
        ...
        <div className="auth-modal-backdrop" role="presentation" onClick={() => { setShowSuspendModal(false); navigate('/') }}>
          ...
          >
            ...
            <div className="auth-modal-actions" style={{ gridTemplateColumns: '1fr' }}>
              <button type="button" className="auth-modal-button" onClick={() => { setShowSuspendModal(false); navigate('/') }}>
                我知道了
              </button>
            </div>
          </div>
